...
public class StringTest {
	...
	public static void main(String[] args) {
		...
		byte[] ascii = str.getBytes(); //문자열을 아스키코드를 구하여 배열로 리턴해준다.
			...
			System.out.println("indexOf->"+ str.indexOf("a")); //처음 만나는 문자위치 index
			System.out.println("lastindexOf->"+ str.lastIndexOf("a")); //뒤에서 처음 만나는 문자위치 index
			System.out.println("indexOf->"+ str.indexOf("z")); //찾는 문자가 없으면 -1
			
			System.out.println("length->"+ str.length()); //글자수
		...
		// 일부문자열 얻어오기
			...
			System.out.println("substring->"+ str.substring(7, 12));// 첫 번째 index부터 두 번째 index앞문자까지의 문자열을 구한다.
			
		//양쪽의 공백문자를 지운다.	
			String txt = "		자바	  프로	그래밍		";
			System.out.println("trim->"+ txt.trim()+ "*");
		
		//문자열의 비교 : equals() : 대소문자 구별함, equalsIgnoreCase() -: 대소문자 구별안함
			String data1 = "JAVA";	// 65 - 97
			String data2 = "Jcmes"; // 
			
			System.out.println("equals()->"+ data1.equals(data2)); //false
			System.out.println("equalsIgnoreCase()->"+ data1.equalsIgnoreCase(data2)); //true
			
		//문자열의 크기비교 : comepareTo() : 대소문자 구별함, compareToIngnoreCase() : 대소문자 구별안함
